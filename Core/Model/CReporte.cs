...
namespace Core.Model
{
    public class CReporte
    {
        ...
        public bool ListarDatosFiltro(int opcion, ref DataSet objDatos, string fecha)
        {
            bool ExisteDatos = false;
            List<SqlParameter> lstParametros = new List<SqlParameter>();
            lstParametros.Add(new SqlParameter("@Op", SqlDbType.Int) { Value = opcion });
            lstParametros.Add(new SqlParameter("@anioIngreso", SqlDbType.VarChar) { Value = fecha });
            objDatos = objManagerBD.GetData("spReporte", lstParametros.ToArray());
            if (objDatos.Tables.Count > 0)
            {
                ExisteDatos = true;
        ...
        public bool listarAño(int opcion, ref DataSet objDatos)
        {
            bool ExisteDatos = false;
            List<SqlParameter> lstParametros = new List<SqlParameter>();
            lstParametros.Add(new SqlParameter("@Op", SqlDbType.Int) { Value = opcion });
